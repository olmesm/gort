...
from gort.models import ShortURL, Webhook, WebhookDelivery, utcnow
	...
	def start(self):
		...
		for job, interval in jobs:
			thread = threading.Thread(
				target=self.loop, args=(job, interval), daemon=True, name=f"gort-{job.__name__}"
			)
			thread.start()
			...
			for link in links:
				...
				try:
					response = outbound.request(
						link.long_url,
						headers={"User-Agent": "gort-title-resolver/1.0"},
				...
				try:
					...
					response = outbound.request(
						...
						headers={
							...
							"X-Gort-Event": delivery.event,
							"X-Gort-Signature": "sha256=" + signature,
						},
						allow_private=self.settings.allow_private_outbound,
						max_bytes=4096,
					)
