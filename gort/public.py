...
from gort.domain import anonymize_ip, check_active, detect_device, forward_query, resolve_target
from gort.models import Domain, RedirectCondition, RedirectRule, ShortURL, Visit, utcnow
...
def record_visit(request, session, kind, link=None, domain=None):
	...
	try:
		session.add(visit)
		session.commit()
		from gort.workers import enqueue_event
