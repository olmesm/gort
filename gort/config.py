...
from gort.domain import validate_domain


class Settings(BaseSettings):
	model_config = SettingsConfigDict(env_prefix="GORT_", extra="ignore")

	port: int = 8080
	default_domain: str = ""
	use_https: bool = False
	data_dir: Path = Path("data")
	db_connection: str = "postgresql://localhost/gort"
	...
	oidc_admin_group: str = "gort-admins"
	...
	def database_connection(cls, value: str) -> str:
		...
		if not value or "://" in value and not value.startswith(("postgresql://", "postgres://")):
			raise ValueError(
				"GORT_DB_CONNECTION must be a PostgreSQL URL or libpq connection string"
			)
		try:
	...
	def defaults(self):
		...
		if self.oidc_issuer and not self.oidc_client_id:
			raise ValueError("GORT_OIDC_CLIENT_ID is required with GORT_OIDC_ISSUER")
		return self
