...
async def oidc_callback(request: Request):
	...
	if not cfg.oidc_enabled:
		raise HTTPException(404)
	state = auth.verify_payload(auth.session_key(request), request.cookies.get("goto_oidc", ""))

	def finish(response):
		response.delete_cookie(
			"goto_oidc", path="/admin/oidc", httponly=True, secure=cfg.use_https, samesite="lax"
		)
		return response
