...
|---|---|---|
| Sessions and API keys | `goto/auth.py` signs cookies, reloads user roles, invalidates local sessions after password changes and checks key expiry. | Authentication tests cover tampering, revocation, expiry, unknown roles and group restrictions. |
| OIDC | `goto/ui.py` uses PKCE, nonce and signed ID-token validation. Sessions end by token expiry. | A signed RSA mock provider exercises discovery, code exchange, JWKS verification, provisioning and callback replay rejection. |
| Browser and proxy protection | `goto/middleware.py` checks form origins and trusted proxy hops, limits bodies and applies security headers and per-IP rate limits. | Public-route tests cover rejected cross-origin forms, headers and untrusted forwarded IPs. |
| Outbound HTTP | `goto/outbound.py` checks DNS answers and connects to the checked address, including on redirects. | Worker tests cover blocked addresses, mixed DNS answers, connection pinning and persistent webhook retries. |
...
framing and referrer protections. Set `GOTO_USE_HTTPS=true` to mark session
...
- OIDC group changes and identity-provider logout take effect at re-login or
  ...
  back-channel logout.
- `GOTO_ALLOW_PRIVATE_OUTBOUND=true` permits internal destinations for title
...
Set `GOTO_TEST_POSTGRES_DSN` to a disposable PostgreSQL database and run
`./scripts/check.sh`. Tests create and remove temporary schemas. Browser checks run
through `./scripts/test-browser.sh`. The [local identity-provider stack](../e2e/keycloak-smoke/README.md)
